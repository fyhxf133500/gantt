...
export function AppShell({
  ...
  const handleMenuButtonClick = (event: MouseEvent<HTMLButtonElement>, projectId: string) => {
    ...
    const top = Math.min(window.innerHeight - 96, rect.bottom + 4);
  ...
  const handleProjectContextMenu = (event: MouseEvent<HTMLDivElement>, projectId: string) => {
    ...
    const top = Math.min(window.innerHeight - 96, event.clientY);
            ...
            <div className="project-list">
              {projects.map((project) => (
                <div
                  key={project.id}
                  className={
                    project.id === activeProjectId ? "project-list-item project-list-item--active" : "project-list-item"
                  }
